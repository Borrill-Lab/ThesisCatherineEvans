...
setwd("C:/Users/evansc/OneDrive - Norwich BioScience Institutes/NAC transgenics/2021-11-10 NAC5 NAP Kronos greenhouse/KASP genotyping 2022-02-15")
n_plates <- 1
layout_samples_names <- c("2022-02-23_Kronos_layout_Plate4.csv")
KASP_raw_names <-  c("2022-02-22_Kronos_Plate4_35c.csv")

use_layout <- FALSE
primers = c("PB479") #use this if the whole plate has the same primer
# layout_primers_names <- "2022-02-17_Kronos_primers_Plate3.csv" #use this if there are different primers
...
for (plate in 1:n_plates){
  ...
  KASP_plate <- read_csv(KASP_raw_names[plate], skip = 18) #18 + 2 for extra plate in the same raw file?
...
#WRITE
write_csv(combined, file = paste(lubridate::today(), "_KASP_Kronos_NAC5_NAP_Plate4.csv"))
write_csv(calls_table_NAC5, file = paste(lubridate::today(), "_KASP_Kronos_NAC5_Plate4_calls.csv"))
write_csv(calls_table_NAP, file = paste(lubridate::today(), "_KASP_Kronos_NAP_Plate4_calls.csv"))
