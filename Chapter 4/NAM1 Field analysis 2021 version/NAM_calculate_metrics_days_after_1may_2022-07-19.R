...
Wide_data <- read_csv("Field_results_NAM_2021_wide_2023-08-23.csv")
...
auc_limit <- Long_data %>% filter(Date == max(Date) & Degree_days_after_heading != 0) %>%
  summarise(
...
write_csv(Wide_data_Degree_days_after_heading, file = paste(table_name, "2021_", "wide_DDAH_", lubridate::today(), ".csv", sep = ""))
