...
#'
#'31/07/2023
#'Update analysis calculation etc. according to data_preparation_2023-06-15
#'
...
NAM2_phenotyping_X50 <- read_excel(paste("NAM2_phenotyping_results", "_", date, ".xlsx", sep= ""), sheet = "X50",
...
NAM2_phenotyping_X61 <- read_excel(paste("NAM2_phenotyping_results", "_", date, ".xlsx", sep= ""), sheet = "X61",
                               col_types = c("numeric", "numeric", "numeric", 
                                             "numeric", "text", "numeric", "numeric", 
                                             "text", "numeric", "text", "text", 
                                             "text", "numeric", "numeric", "date", 
...
NAM2_phenotyping_clean <- NAM2_phenotyping %>%
  mutate(Snapped_pre_peduncle_sen = if_else(Snapped_pre_peduncle_sen %in% c('T','t',TRUE,'TRUE'), true = TRUE, false = FALSE),
         across(c(`SPAD_2023-01-13`, `SPAD_2023-01-16`, `SPAD_2023-01-19`, `SPAD_2023-01-23`, `SPAD_2023-01-27`), ~ replace_na(.x, 0))
         )
...
#Fix weird columns
NAM2_phenotyping_yday <- NAM2_phenotyping_yday %>%
  mutate(`SPAD_2022-11-23` = ifelse(is.na(`SPAD_2022-11-23`) & Scoring_day == "MON", `SPAD_2022-11-26 and 23`, `SPAD_2022-11-23`),
         `SPAD_2022-11-26` = ifelse(is.na(`SPAD_2022-11-26`) & Scoring_day == "FRI", `SPAD_2022-11-26 and 23`, `SPAD_2022-11-26`),)
